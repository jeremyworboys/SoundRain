...
SoundRain.prototype.test = function(callback) {
	...
	try {
		...
		this.emit('error', new Error('Incorrect write permissions on directory'));
	}
...
SoundRain.prototype.find = function() {
	var tracks, self = this, trackData = '';
	...
	http.get(this.url, function(res) {
		...
		res.on('end', function() {
			tracks = trackData.match(/(window\.SC\.bufferTracks\.push\().+(?=\);)/gi);
			self.download(self.parse(tracks[0]));
		});
	});
...
SoundRain.prototype.parse = function(raw) {
	...
	try {
		...
		this.emit('error', new Error('Couldn\'t parse URL.'));
	}
